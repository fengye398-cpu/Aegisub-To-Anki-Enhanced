...
# -*- coding: utf-8 -*-
"""
Aegisub_to_Anki打包配置脚本
使用PyInstaller打包Aegisub_to_Anki为单文件exe
"""
...
class Aegisub_to_AnkiBuilder:
    """Aegisub_to_Anki打包配置管理器"""

    def __init__(self):
        self.app_name = "Aegisub_to_Anki"
        self.version = "1.0.0"
    ...
    def get_pyinstaller_args(self, show_console=True):
        ...
        args = [
            ...
            "--onefile",  # 打包成单个exe文件
            "--clean",    # 清理临时文件
            "--hidden-import", "psutil",  # 确保psutil被打包（用于动态线程调整）
        ]
        ...
        # 图标设置
        icon_file = self.base_dir / "Aegisub_to_Anki.ico"
        if icon_file.exists():
            args.extend(["--icon", str(icon_file)])

        # 主程序入口
        args.append(str(self.base_dir / "Aegisub_to_Anki.py"))
    ...
    def check_dependencies(self):
        ...
        # 检查主程序
        main_file = self.base_dir / "Aegisub_to_Anki.py"
        if main_file.exists():
    ...
    def build_application(self, show_console=True):
        ...
        try:
            ...
            console_mode = "显示控制台" if show_console else "隐藏控制台"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Aegisub_to_Anki打包工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python build_Aegisub_to_Anki.py              # 打包带控制台的exe
  python build_Aegisub_to_Anki.py --no-console # 打包无控制台的exe
  python build_Aegisub_to_Anki.py --clean      # 清理构建文件
  python build_Aegisub_to_Anki.py --check      # 仅检查依赖
        """
    )
    parser.add_argument(
        "--no-console",
    ...
    builder = Aegisub_to_AnkiBuilder()
